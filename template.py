...
## Create engine using the SQL database file

#For SQLite use following.
#engine = create_engine("sqlite:///../Resources/top50playlist.sqlite")

# For MySQL use following.
connect_string = "root:root@127.0.0.1/music"
engine = sqlalchemy.create_engine('mysql+pymysql://root:root@127.0.0.1') # connect to server
engine.execute("CREATE DATABASE music") #create db
engine.execute("USE music")
engine = create_engine(f'mysql+pymysql://{connect_string}')

## Read Top 50 Playlist by Country Excel to get URL for each country
...
for playlist_id in top50_playlist_df["Spotify Playlist ID"]:
    ...
    for i in range(number_of_tracks):
        ...
        except (KeyError, IndexError, InternalError): ## Figure out how to except InternalError , SQLAlchemyError
            #danceability.append("NaN")
            #energy.append("NaN")
            track_name.append(playlist_tracks["Null"])
    ...
    artist_country_df.to_sql(name=f'{country}', con=engine, if_exists='append', index=True)
    
    counter += 1
    
print("--------------------------\nDone Converting JSON File\n--------------------------")